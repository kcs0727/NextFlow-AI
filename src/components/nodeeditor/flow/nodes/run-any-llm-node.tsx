...
function TextField({
    ...
    return (
        <label className="block space-y-1">
            <span className="text-[11px] uppercase tracking-[0.14em] text-slate5">{label}</span>
            <textarea
                value={value}
                ...
                rows={rows}
                className="w-full resize-none rounded-xl border border-slate7 bg-slate95 px-3 py-2 text-sm text-slate2 outline-none transition focus:border-slate5 disabled:cursor-not-allowed disabled:opacity-40"
            />
        </label>
    );
}
    ...
    return (
        <NodeShell title={typedData.title} status={typedData.status} error={typedData.error} selected={selected}>
            ...
            <div className="space-y-3">
                {/* Model Selector */}
                <label className="block space-y-1">
                    <span className="text-[11px] uppercase tracking-[0.14em] text-slate5">Model</span>
                    <select
                        value={typedData.values.model ?? "gemini-2.5-flash"}
                        onChange={(e) => updateNodeValue(id, "model", e.target.value)}
                        className="w-full rounded-xl border border-slate7 bg-slate95 px-3 py-2 text-sm text-slate2 outline-none transition focus:border-slate5"
                    >
                ...
                {/* Images Info */}
                <div className="text-[11px] text-slate5">
                    {imagesConnected ? (
                ...
                {imagePreviewUrls.length > 0 && (
                    <div className="space-y-2">
                        <p className="text-[11px] uppercase tracking-[0.14em] text-slate5">Connected Images</p>
                        <div className="grid grid-cols-3 gap-2">
                            {imagePreviewUrls.map((url, index) => (
                                <img
                                    ...
                                    src={url}
                                    alt={`Input ${index + 1}`}
                                    className="h-16 w-full rounded-lg border border-slate7 bg-slate95 object-cover"
                                />
                            ))}
                        </div>
                    </div>
