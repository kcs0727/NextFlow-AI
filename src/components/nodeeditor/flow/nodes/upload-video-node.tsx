...
import { uploadViaTransloadit } from "@/lib/workflow/transloadit";
...
function UploadVideoNode({ id, data, selected }: NodeProps) {
  ...
  const url = typedData.values.video_url ?? "";
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      updateNodeValue(id, "video_url", result.url);
    } catch (error) {
  ...
  return (
    <NodeShell title={typedData.title} status={typedData.status} error={typedData.error} selected={selected}>
      <button
        ...
        className="flex items-center justify-center gap-2 rounded-xl bg-slate95/50 px-3 py-4 text-sm text-slate4 transition hover:border-slate5 hover:bg-slate95 disabled:opacity-50 disabled:cursor-not-allowed"
      >
      ...
      {url && (
        <div className="space-y-2">
          <video src={url} controls className="h-32 w-full rounded-xl border border-slate7 bg-slate95 object-cover" />
          <a
            href={url}
            target="_blank"
            rel="noreferrer"
      ...
      <OutputHandle id="output" top="85%" label="video" kind={typedData.kind} />
    </NodeShell>
  );
}
